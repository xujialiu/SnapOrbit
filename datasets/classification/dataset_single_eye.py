...
class LoadData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = self.data_path / img_name.split("::", 1)[0]
        ...
        eye_side = self.df.item(index, EYE_SIDE_COL)
        inner_boundary_x_pct = self.df.item(index, INNER_BOUNDARY_COL)
        ...
        # Append ::R/::L suffix so results_<n>.csv distinguishes per-eye samples
        return (img_tensor, f"{img_path}::{eye_side}", label)
