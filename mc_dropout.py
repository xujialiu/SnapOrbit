...
class MCDropout(nn.Module):
    ...
    def fit_regression(self, x: Tensor) -> tuple[Tensor, Tensor]:
        """Run MC Dropout and return mean prediction with uncertainty.

        Args:
            x (Tensor): input tensor of shape (B, ...)

        Returns:
            mean_pred (Tensor): mean prediction of shape (B, T)
            uncertainty (Tensor): per-target uncertainty in (0, 1) of shape (B, T)
        """
        B = x.shape[0]
        raw = self.forward(x)  # (E*B, T)
        preds = raw.view(self.num_estimators, B, -1)  # (E, B, T)
        mean_pred = preds.mean(dim=0)  # (B, T)
        variance = preds.var(dim=0)  # (B, T)
        eps = 1e-8
        h = 0.5 * torch.log(2 * math.pi * math.e * variance + eps)
        uncertainty = torch.sigmoid(h)  # (B, T), in (0, 1)
